...
class DataProcess:
    ...
    # Generate dataset based on csv_path
    def dataset_generation(self, csv_path):
        # Initialize input, label
        # Read csv file
        X = [] # X list is for data
        Y = [] # Y list is for label
        ...
        # angular encoder encodes angle in degrees into sin/cos values to avoid phase wrapping
        if self.dataset_name == 'umd':
            if self.mode == 'position':
                ...
                if self.separation is not None:
                    dataset = dataset[:, self.separation]
                dataset = Utils.angular_encoder(dataset)
            else:
        ...
        # Generate dataset using our custom Dataset class
    ...
    #Genetate dataloader used in training and testing process
    def dataloader_generation(self, train_index, test_index, test_ratio=0, valid_ratio=0):
        trainset_list = []
        testset_list = []
        validset_list = []
        if self.dataset_name == 'umd':
        ...
        valid_examples = int(len(trainset) * valid_ratio)
        test_examples = int(len(trainset) * test_ratio)
        train_examples = len(trainset) - valid_examples - test_examples
        trainset, validset, testset = data.random_split(trainset, [train_examples, valid_examples, test_examples])
